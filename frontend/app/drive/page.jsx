...
export default function DrivePage() {
  ...
  return (
    <div>
              ...
              >
                ...
                <button
                  ...
                  className='absolute top-2 right-2 opacity-0
                             group-hover:opacity-100 text-gray-400
      ...
      {/* Files list — removed overflow-hidden so dropdown is never clipped */}
      {files.length > 0 && (
        <div>
          <h2 className='text-sm font-medium text-gray-500 uppercase
                         tracking-wide mb-3'>
...
function FileRow({ file, isFirst, isLast, onDelete, onRename }) {
  ...
  // ── Open file in new tab (for clicking file name / icon) ─────────────
  ...
  // ── Force download via blob (bypasses cross-origin restriction) ──────
  // Supabase signed URLs are on a different domain (*.supabase.co),
  // so the browser ignores <a download> for them. Fetching as a blob
  // first creates a same-origin blob:// URL that the browser WILL download.
  async function handleDownload() {
    ...
    try {
      ...
      if (data.signedUrl) {
        // Step 2: fetch the actual file bytes as a blob
        const blobRes = await fetch(data.signedUrl);
        const blob    = await blobRes.blob();

        // Step 3: create a local blob:// URL and click it
        ...
        // Step 4: release the object URL from memory
  ...
  function handleMenuToggle() {
    ...
    if (btnRef.current) {
      ...
      const menuHeight = 220; // approximate height of the full menu
    ...
    >
      ...
      >
        ...
        <div className='flex flex-col gap-0.5 mt-1'>
          <div className={`w-5 h-0.5 rounded opacity-40
                           ${fileColor.text} bg-current`} />
          <div className={`w-4 h-0.5 rounded opacity-40
                           ${fileColor.text} bg-current`} />
          <div className={`w-5 h-0.5 rounded opacity-40
                           ${fileColor.text} bg-current`} />
        </div>
      ...
      <div className='relative'>
        ...
        {/* Dropdown — rendered via portal-like fixed positioning so it
            is never clipped by any parent overflow:hidden              */}
        {showMenu && (
          <>
            ...
            >
              {/* Download — blob trick forces save dialog on all domains */}
              <button
                onClick={() => { setShowMenu(false); handleDownload(); }}
                className='w-full flex items-center gap-3 px-4 py-2.5
                           text-sm text-gray-700 hover:bg-gray-50
                           transition-colors text-left rounded-t-xl cursor-pointer'
              >
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'
                  fill='none' stroke='currentColor' strokeWidth='1.5'
                  ...
                  <path strokeLinecap='round' strokeLinejoin='round'
                    d='M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25
                       2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5
                       12m4.5 4.5V3' />
                </svg>
                Download
              </button>
                ...
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'
                  ...
                  <path strokeLinecap='round' strokeLinejoin='round'
                    d='m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652
                       2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8
                       -2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0
                       0L19.5 7.125' />
                </svg>
                Rename
              </button>
                ...
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'
                  ...
                  <path strokeLinecap='round' strokeLinejoin='round'
                    d='M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0
                       0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037
                       .666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576
                       -5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971
                       5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12
                       12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0
                       0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197
                       a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6
                       0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25
                       2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0
                ...
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'
                  ...
                  <path strokeLinecap='round' strokeLinejoin='round'
                    d='M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5
                       4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757
                       -1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5
                       0 0 0 1.242 7.244' />
                </svg>
                 Link
              </button>
              ...
              <button
                ...
                className='w-full flex items-center gap-3 px-4 py-2.5
                           ...
                           transition-colors text-left rounded-b-xl cursor-pointer'
              >
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'
                  fill='none' stroke='currentColor' strokeWidth='1.5'
                  className='w-4 h-4'>
      ...
      {showLink && (
        <LinkModal file={file} onClose={() => setShowLink(false)} />
      )}
    </div>
  );
}